...
const { getAIProvider } = require('../services/ai/AIProvider.js');
const { buildSystemContext, SYSTEM_PROMPT } = require('../services/ai/contextBuilder');

const MAX_MESSAGE_LENGTH = 2000; // basic abuse/cost guard, per spec Section 23
...
router.post('/chat', authMiddleware.requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Aqua AI is temporarily unavailable. Please try again.' });
  }
});
